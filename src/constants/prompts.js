...
8. 소스 팩트 완전 반영 (절대 규칙):
   - 소스의 모든 숫자(금액, 기간, 수량, 날짜)를 빠짐없이 본문에 반영.
   - 생성 완료 후, 소스 원문의 숫자를 하나씩 대조하여 누락이 있으면 반드시 삽입하라.
   - 예: 소스에 "3년 계약, 연 300대 규모"이면 → "3년"과 "연 300대" 모두 본문에 있어야 함.
   - 쌍으로 나오는 팩트(기간+수량, 금액+기간 등)는 반드시 함께 반영. 하나만 넣고 다른 하나를 빼면 안 됨.
...
### 7. [FACT] 팩트 완전성/중복/품질
- 소스 원문의 숫자(기간, 수량, 금액, 날짜)를 하나씩 본문에서 검색하여 누락 여부 대조.
  - 누락 발견 시 → severity: "red", category: "팩트 누락", message에 누락된 구체적 숫자/기간 명시. 자동 수정으로 누락된 팩트 삽입.
  - 쌍으로 나오는 팩트 중 하나만 반영된 경우도 누락으로 처리 (예: "3년 계약, 연 300대" 중 "연 300대"만 있고 "3년"이 없으면 누락)
...
2. 수정 가능한 이슈:
   ...
   - [FACT] "팩트 누락" → 확인된 팩트에서 누락된 숫자/기간/수량을 찾아 본문의 관련 문단에 자연스럽게 삽입. 예: "연 300대"만 있고 "3년"이 빠졌으면 → "3년간 연 300대 규모"처럼 쌍으로 반영
