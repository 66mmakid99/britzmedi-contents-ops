...
- 분량: 2,500~3,500자 (소스 분량에 비례 — 소스가 짧으면 짧게)
- 구조: 아래 섹션 라벨을 반드시 사용하여 구분
  [핵심키워드] 2~3개
  [보조키워드] 5~7개
  [제목] 질문형/How-to형, 30~40자, 핵심 키워드 앞배치
  [서론] 첫 200자: 질문 → 대상독자 → 핵심포인트 2~3개
  [소제목1] 질문형 소제목 + 4단 답변 (한줄답→정의→사례→FAQ)
  [소제목2] 질문형 소제목 + 4단 답변
  [소제목3] 질문형 소제목 + 4단 답변
  (필요시 [소제목4], [소제목5] 추가)
  [결론] 요약 + 키워드 재언급 + CTA + 내부 링크
  [태그] 태그 7~12개 (쉼표 구분)
- 키워드: 핵심 2~3개(제목·서론·결론) + 보조 5~7개(본론)
- 이미지: [IMAGE: alt 텍스트 20~50자, 키워드 포함] 10~15개
- 짧은 문단 (3~5문장, 300~500자)
- 전문용어에 괄호 설명 병기
- "뷰티 디바이스" 표현 금지 → "전문 의료기기" 또는 "Medical Device" 사용
...
8. CTA 링크 삽입 (인스타그램 제외):
   본문의 CTA/마무리 부분에 반드시 아래 두 플레이스홀더를 포함하세요.
   📋 데모 신청하기: {DEMO_LINK}
   💬 제품 상담하기: {CONSULT_LINK}
   {DEMO_LINK}와 {CONSULT_LINK}를 정확히 그대로 작성하세요. 실제 URL은 시스템이 자동 치환합니다.
   인스타그램은 링크 삽입이 불가하므로 "프로필 링크에서 데모 신청 & 제품 문의 가능!" 텍스트만 넣으세요.`;

  const channelPrompts = {
    'newsletter': `당신은 피부과/에스테틱 원장님을 대상으로 하는 뉴스레터 전문 작가입니다.
보도자료를 그대로 옮기지 마세요. 원장님 관점에서 재해석하세요.
(상세 가이드: docs/EMAIL-NEWSLETTER-GUIDELINE.md 참조)
...
인사이트: 해외 검증→국내 신뢰도, 글로벌 임상 데이터, 장비 선택 참고 포인트
...
[CTA]
⑤ Primary CTA — 이번 호의 가장 중요한 행동 1개 (2~3문장)
따뜻한 마무리 인사 후 자연스럽게 행동 유도.
예: "토르RF에 대해 더 자세히 알고 싶으시다면, 담당자에게 편하게 문의해주세요."
예: "이런 인사이트를 정기적으로 받고 싶으시면 이 이메일에 회신해 주세요."
...
(상세 가이드: docs/NAVER-BLOG-GUIDELINE.md 참조)
...
- 한 문장: 30~50자 (한글 기준)
- 한 단락: 300~500자 (3~5문장)
- 전체 글: 2,500~3,500자 (반드시 2,500자 이상. 2,500자 미만이면 출력 무효)
- 소제목 수: 3~5개
...
템플릿: "이 글에서는 '[질문]'에 대해 정리합니다. [대상 독자]를 위해, ① [포인트1], ② [포인트2], ③ [포인트3] 순서로 설명하겠습니다."

[IMAGE: 대표 이미지 alt 텍스트]

[소제목1]
질문형 소제목 (실제 사용자 검색 의도 반영)
소제목 패턴: "~은 무엇인가요?", "~은 어떻게 하나요?", "왜 ~이 중요한가요?"
아래 4단 답변 패턴을 반드시 따르세요:
1) 한 줄 직접 답 — 핵심 결론을 먼저 제시
2) 정의/원리 — 배경 설명 2~3문장
3) 사례/숫자/경험 — 구체적 데이터, 실제 사례
...
[분량] 반드시 300자 이상, 500자 이내로 작성하세요. 300자 미만이면 출력 무효입니다.
팩트 전달 후 '이 소식의 의미' 또는 '구독자에게 주는 가치'를 1~2문장으로 반드시 포함하세요.
예: "국내 기술로 만든 장비가 해외 피부과에서 선택받았다는 건, 그만큼 기술력이 인정받고 있다는 뜻이에요."
...
- 팔로우 유도: "의료기기 수출 인사이트를 계속 보고 싶다면 팔로우해 주세요."
외부 링크가 있을 경우 "관련 링크는 첫 댓글에 남겨둡니다" 한 줄 추가.

[첫 댓글용 링크]
외부 링크가 필요한 경우에만 작성 (없으면 이 섹션 생략).
예: "🔗 토르RF 상세 정보: www.britzmedi.co.kr"
