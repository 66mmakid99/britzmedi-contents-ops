...
### 작성 방법 — 5단 역피라미드 구조
1단 [리드문]: "메디컬 에스테틱 디바이스 전문기업 브릿츠메디(대표 이신재)는..." + 확인된 팩트의 핵심 뉴스 (육하원칙, 2~3문장)
2단 [배경]: 핵심 팩트의 배경·맥락. 지식베이스/회사정보로 보강하되, 이번 뉴스와 직접 관련된 내용만.
3단 [구체]: 상세 내용 전개 — 계약 조건, 기술 스펙, 행사 프로그램 등 팩트 중심.
4단 [의미+인용문]: 이번 뉴스의 의미와 대표 인용문을 자연스럽게 녹여 넣는다.
   - 확인된 팩트에 인용문이 있으면 그대로 사용.
   - 없으면 AI가 "이신재 브릿츠메디 대표는 '성과/자부심 톤의 인용문'이라고 밝혔다." 형태의 기본 인용문(A안)을 직접 생성하여 삽입.
   - [QUOTE_PLACEHOLDER], [대표 인용문], [인용문] 등 플레이스홀더 절대 사용 금지. 반드시 완성된 인용문을 넣어라.
   - 인용문 문단은 반드시 본문의 일부로 서술체로 이어져야 한다. 별도 섹션으로 분리하지 마라.
5단 [마무리]: 향후 계획 또는 핵심 메시지 정리 (소스에 향후 계획 있을 때만)
...
[
  { "label": "A", "tone": "성과/자부심", "text": "${speaker} 브릿츠메디 ${title}는 '인용문 내용'이라며 '인용문 내용'이라고 밝혔다." },
  { "label": "B", "tone": "비전/미래", "text": "${speaker} 브릿츠메디 ${title}는 '인용문 내용'이라고 말했다." },
  { "label": "C", "tone": "파트너/시장", "text": "${speaker} 브릿츠메디 ${title}는 '인용문 내용'이라고 전했다." }
]`;
}
