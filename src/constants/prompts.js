...
4. 인용문 문단 (4번째 문단): 대표 인용문을 본문 흐름 안에 자연스럽게 녹여 넣는다.
   - 확인된 팩트에 인용문이 있으면 그대로 사용.
   - 없으면 [QUOTE_PLACEHOLDER] 마커를 4번째 문단 위치에 삽입 (나중에 선택된 인용문으로 대체됨).
   - 인용문 문단은 반드시 본문의 일부로 서술체로 이어져야 한다. 별도 섹션([대표 인용문])으로 분리하지 마라.
5. 마무리/향후 계획: 핵심 메시지 정리 또는 향후 계획
6. 알 수 없는 정보: [추가 정보 필요: 구체적 설명] 플레이스홀더
7. 분량 (엄격): 목표 ${channelConfig.charTarget}. 단, 소스 분량에 비례.
