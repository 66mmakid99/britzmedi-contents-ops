...
[원본 보도자료 본문]
${pressRelease.body || pressRelease.draft || ''}
...
5. 영문 표기: 첫 등장 시 '국문(영문)', 이후 국문만 (약어 예외: FDA, CE, ISO).
6. "뷰티 디바이스" 표현 금지 → "전문 의료기기" 또는 "Medical Device" 사용.
7. 채널별 설문 데이터 사용 규칙:
   - 카카오/인스타그램: 설문 데이터 완전 금지
   - 네이버/뉴스레터: 맥락이 있을 때만, 반드시 출처 명시
   - LinkedIn: 출처 명시하면 OK (1~2개만, 3개 이상 연속 금지)
8. 채널별 팩트 비율 기준: 보도자료 100%, 블로그 80%, LinkedIn 70%, 카카오/인스타 해당없음
9. [카드1], [캐러셀1], [슬라이드1] 같은 구조 태그 출력 금지
`;
...
요구사항:
1. 제목: SEO 키워드 앞배치, 매력적인 블로그 제목 (보도자료 제목과 다르게)
2. 소제목: 3~5개의 소제목으로 구조화 (뉴스상세/기술해설/시장의미)
3. 본문: 각 소제목 아래 2~3개 문단. 전문적이되 접근성 있는 톤.
4. 이미지 위치: 본문 내에 [IMAGE: 설명] 위치 표시
5. SEO 키워드: 5~8개 추출, 제목 앞부분에 핵심 키워드 배치
6. CTA: 마지막에 자연스러운 행동 유도 문구
7. 태그: 8~12개
8. 설문 데이터: 맥락이 있을 때만, 반드시 출처 명시
...
요구사항:
1. 제목: 핵심 뉴스 한 줄 (20자 이내)
2. 본문: 핵심 메시지 → 배경 → 제품/기술 → 의미/전망 순서로 자연스럽게 전개
3. CTA: 행동 유도 문구 + 링크 안내
4. 이모지: 0~2개 (절제)
5. 설문 데이터: 완전 금지
6. 전문용어 최소화
7. [카드1], [카드2] 같은 구조 태그 출력 금지
...
요구사항:
1. 캡션: 핵심 메시지 1~2문장. 첫 줄에 훅. 짧고 임팩트 있게.
2. 이미지 가이드: 피드 이미지 추천 (구도, 텍스트 오버레이 등) — 참고용
3. 해시태그: 15~20개 (23개 이상 스팸 주의, 한글+영어)
4. 이모지: 캡션 3개 이내
5. 설문 데이터: 완전 금지
6. [캐러셀1], [슬라이드1] 같은 구조 태그 출력 금지
...
2. 본문: 전문가 관점에서 산업 인사이트를 녹여서 작성 (3~4문단)
3. 핵심 포인트: 3가지 불릿
4. 톤: 비즈니스 전문가가 자사 소식을 공유하는 느낌
5. 구조: 훅 → 맥락 → 핵심 소식 → 의미/인사이트 → CTA
6. 줄바꿈: 링크드인 특성상 짧은 문단 + 빈 줄 활용
7. 마무리: 인게이지먼트 질문
8. 해시태그: 5~8개 (한글+영어)
9. 이모지: 0~2개
10. 설문 데이터: 출처 명시하면 OK (1~2개만, 3개 이상 연속 금지)
