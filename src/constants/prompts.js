...
- 분량: 2000~3000자
- 구조: SEO 제목(키워드 앞배치, 40자) → 공감형 도입부 → 소제목 3~5개(데이터→인사이트→팁) → [이미지: 설명] 4곳 이상 → 핵심요약+CTA → 태그 10개
- SEO 키워드 자연 배치
- 짧은 문단 (3~4줄)
- 전문용어에 쉬운 설명 병기
- 이미지 생성 프롬프트 3개 제공
...
### 구조 (반드시 이 순서로):
1. [보도자료] 표기
2. 제목: 핵심 뉴스가 한 줄에 담긴 제목 (30~40자). "브릿츠메디, [핵심 뉴스]" 형식. 숫자, 구체적 사실 포함.
3. 부제: 제목을 보완하는 한 줄 (40~50자)
4. 리드문: 육하원칙 포함, 2~3문장으로 전체 요약. "메디컬 에스테틱 디바이스 전문기업 브릿츠메디(대표 이신재)는..." 으로 시작
5. 본문 (3~4문단): 핵심 상세 → 배경/맥락 → 관계자 인용문 → 향후 계획
6. 인용문: "이신재 브릿츠메디 대표는 '인용 내용'이라며 '추가 인용'이라고 밝혔다."
7. 회사 소개 보일러플레이트:
...
- 마지막에 [이미지 가이드]: 보도자료와 함께 배포할 이미지 2~3장 제안`,
  },
};
